import { a as attr, a9 as head, b as ensure_array_like, aa as attr_class, e as escape_html, ab as stringify, ac as attr_style, d as derived } from "../../chunks/index.js";
...
function Hero($$renderer) {
  ...
  $$renderer.push(`<!---->  <div class="container-page pointer-events-none relative z-10 pt-[180px] text-center md:pt-[220px]"><h1 class="mx-auto max-w-3xl text-web-32 font-semibold text-white sm:text-web-40 md:text-web-48 lg:text-web-56">Payments Infrastructure<br/>Built for Modern Businesses.</h1> <p class="mx-auto mt-5 max-w-3xl text-balance text-para-20 text-white/70">Accept payments, automate payouts, and manage your entire payment ecosystem with a secure
			platform designed to help businesses grow faster.</p> <div class="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row sm:gap-4"><a href="/contact" class="pointer-events-auto w-full rounded-lg border border-white/20 bg-white/5 px-5 py-4 text-center font-mono text-para-16 font-medium text-white hover:bg-white/10 sm:w-auto sm:px-8">Get Started</a> <a href="/products" class="pointer-events-auto w-full rounded-lg bg-white px-5 py-4 text-center font-mono text-para-16 font-medium text-black hover:bg-gray-100 sm:w-auto sm:px-8">View Documentation</a></div></div>  <div class="container-page pointer-events-none relative z-10 mt-[120px] pb-16"><div class="relative mx-auto max-w-6xl"><img${attr("src", arrow)} alt="" class="pointer-events-none absolute -top-[176px] right-[-116px] z-20 hidden w-36 opacity-90 md:block lg:right-[-80px] lg:w-44"/> <img${attr("src", dashboard)} alt="ElixirPay dashboard" class="block w-[760px] max-w-none drop-shadow-2xl md:w-full md:max-w-full"/></div></div></section>`);
}
    ...
    for (let i = 0, $$length = each_array_2.length; i < $$length; i++) {
      ...
      if (mobileOpenCategory === i) {
        ...
        for (let j = 0, $$length2 = each_array_3.length; j < $$length2; j++) {
          ...
          if (j === mobileOpenItem) {
            $$renderer2.push("<!--[0-->");
            $$renderer2.push(`<div class="mt-4 border border-gray-200 bg-gray-50 svelte-1uha8ag"><div class="border-b border-gray-200 px-6 py-5 svelte-1uha8ag"><span class="font-mono text-para-16 font-bold uppercase tracking-wide text-gray-900 svelte-1uha8ag">${escape_html(item.label)}</span></div> <div class="p-6 svelte-1uha8ag"><p class="flex gap-2 text-para-16 text-gray-500 svelte-1uha8ag"><span class="mt-0.5 shrink-0 text-gray-400 svelte-1uha8ag">✓</span> <span class="svelte-1uha8ag">${escape_html(item.description)}</span></p> <a href="/products" class="mt-5 inline-block rounded-lg bg-gray-200 px-5 py-2.5 font-mono text-para-14 font-medium text-gray-900 hover:bg-gray-300 svelte-1uha8ag">${escape_html(item.cta ?? item.heading)}</a> `);
